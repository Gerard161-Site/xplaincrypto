...
|------------|------------|-------------|-------------------|-------------------|---------------------|--------------|
| TVL Chart | LineChartVisualizer | DeFiLlama | data.defillama.tvl_history | List of `[timestamp, tvl]` or `[{"date": timestamp, "tvl": value}]` | docs/ondo/cache/defillama/tvl_ondo.json | 24h |
| Price Chart (30-day) | CandlestickChartVisualizer | CoinMarketCap | data.coinmarketcap.ohlcv | List of `[{"date": timestamp, "open": float, "high": float, "low": float, "close": float, "volume": float}]` | docs/ondo/cache/coinmarketcap/ohlcv_ondo_30.json | 1h |
| Volume Chart | LineChartVisualizer | CoinMarketCap | data.coinmarketcap.volume_history | List of `[{"date": timestamp, "volume": float}]` | docs/ondo/cache/coinmarketcap/historical_ondo_days_30.json | 4h |
| Tokenomics Pie Chart | PieChartVisualizer | Tokenomics | data.tokenomics.token_distribution | Nested object with `token_allocation` field containing Dict of `{category: percentage}` | docs/ondo/cache/tokenomics/distribution_ondo.json | 168h |
| Chain Distribution Pie Chart | PieChartVisualizer | DeFiLlama | data.defillama.currentChainTvls | Dict of `{chain: tvl}` or `[{"chain": str, "tvl": float}]` | docs/ondo/cache/defillama/protocol_protocol_ondo.json | 24h |
...
### Cache Structure
- Cache files are organized by source and endpoint: `docs/ondo/cache/{source}/{endpoint}_{query}.json`
